...
public sealed class Error : ICloneable
{
    ...
    [AllowNull]
    public ICollection<ElmahLogSqlEntry> SqlLog
    {
        get => _sqlLog ??= new List<ElmahLogSqlEntry>();
        set => _sqlLog = value?.ToList();
    }

    [AllowNull]
    public ICollection<ElmahLogParamEntry> Params
    {
        get => _params ??= new List<ElmahLogParamEntry>();
        set => _params = value?.ToList();
    }
